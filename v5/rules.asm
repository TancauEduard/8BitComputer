...
#ruledef
{
    ...
    switch a, b => 0x16
    switch b, a => 0x16
    switch {r:register}, {zp:i8} => 0x1 @ (r + 7)`4 @ zp
    switch {r:register}, {addr:i16} => 0x1 @ (r + 9)`4 @ le(addr)
    switch {zp1:i8}, {zp2:i8} => 0x1b @ zp1 @ zp2
    switch {zp:i8}, {addr:u16} => 0x1c @ zp @ le(addr)
    switch {addr:i16}, {zp:i8} => 0x1c @ le(addr) @ zp
    switch {addr1:i16}, {addr2:i16} => 0x1d @ le(addr1) @ le(addr2)
